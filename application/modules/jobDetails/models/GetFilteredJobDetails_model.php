...
class GetFilteredJobDetails_model extends CI_Model
{
    ...
    public function getFilteredJobDetails($fieldvalArr)
    {
        ...
        $sql = "select IFNULL (b.`id`,'') as id,
				...
				IFNULL (group_concat(concat(jt.`job_type`,'|',DATE_FORMAT(jcd.start_date,'%d-%m-%Y'),'|',DATE_FORMAT(jcd.end_date,'%d-%m-%Y'),'|', jcd.job_type_id,'|',IFNULL(jcd.`comment`,''),'|',IFNULL(jcd.`title`,''))),'') as job_type_data,
