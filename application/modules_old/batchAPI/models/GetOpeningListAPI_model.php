...
class GetOpeningListAPI_model extends CI_Model
{
	...
	for($i=0;$i<sizeof($input);$i++){	
		...
		$sql = "INSERT INTO opening_details (openingId,buId,openTitle,crStreamId,crStreamName,employmentType,yrsOfExpMax,yrsOfExpMin,noPositionsTotal,noPositionsClosed,statusId,statusName,
                ctcBandLowEnd,ctcBandHighEnd,ctcCurrency,address,office,locationId,hiringManager,hiringManagerEmpId,coordinator,coordinatorEmpId,hireByDate,createdDate,activationDate,closedDate,updatedDate,API_called_date) values
				( '". $input[$i]['openingId']. "','"
					. $input[$i]['buId']. "','"
					. $input[$i]['openTitle']. "','"
					. $input[$i]['crStreamId']. "','"
					. $input[$i]['crStreamName']. "','"
					...
					. $dt. "' )
					 ...
					 buId = '" .$input[$i]['buId']."',
					 openTitle = '". $input[$i]['openTitle'] ."' ,
					 crStreamId = '" .$input[$i]['crStreamId']."',
					 crStreamName = '". $input[$i]['crStreamName'] ."' ,
					 ...
					 hiringManager = '". $input[$i]['hiringManager'] ."' ,
					 hiringManagerEmpId = '" .$input[$i]['hiringManagerEmpId']."',
					 coordinator = '". $input[$i]['coordinator'] ."' ,
